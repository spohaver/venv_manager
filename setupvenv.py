...
class VirtualEnvironmentSetup:
    ...
    def install_packages(self) -> bool:
        ...
        if not self.requirements_file.exists():
            print(f"No requirements.txt found at {self.requirements_file}, skipping package installation.")
            return True
...
""")
            ...
            print("Run './venv_shell' to activate the virtual environment in a NEW shell")
    ...
    def get_venv_info(self) -> dict:
        ...
        try:
            ...
            # Get directory size
            size = sum(f.stat().st_size for f in self.venv_path.rglob('*') if f.is_file())
    ...
    def remove_environment(self, force: bool = False) -> bool:
        ...
        if not self.venv_path.exists():
            print(f"Virtual environment '{self.venv_name}' does not exist at {self.venv_path}")
            return False
    ...
    def list_environments(base_dir: Path, detailed: bool = False) -> None:
        ...
        for venv_dir in sorted(venv_dirs):
            ...
            if detailed:
                ...
                info = dummy_setup.get_venv_info()
                print(f"  {info.get('name', 'Unknown'):20} | "
...
Examples:
  ...
  %(prog)s remove --name myproject --force    # Remove without confirmation
    ...
    create_parser = subparsers.add_parser('create', help='Create or update virtual environment')
...
def main() -> int:
    ...
    try:
        ...
        if command == 'list':
            ...
            requirements_file = Path.cwd() / "requirements.txt"  # Dummy for remove operation
...
def print_legacy_help():
    ...
    print("""Python Virtual Environment Setup Tool
